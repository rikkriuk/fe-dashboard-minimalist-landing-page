...
import { deletePortfolio, fetchPortfolios } from "../redux/slices/portfolioSlice";
import LoadingComponent from "./LoadingComponent";
import { showSuccessAlert } from "../utils/alert";
import OverlayComponent from "./OverlayComponent";

const PortfolioListComponent = ({ toggleOverlay, isOverlayVisible, overlayImageUrl, setIsOverlayVisible }) => {
  const { portfolios, pagination, error, loading } = useSelector((state) => state.portfolio);
  ...
  useEffect(() => {
    dispatch(fetchPortfolios({ search, page, limit: 5 }));
            ...
            <div className="p-6">
               ...
               <table className="w-full text-dark border-neutral-200">
                  <thead className="text-secondary-dark text-[0.95rem] font-semibold">
                     ...
                     <th className="pb-3 text-gray-600 text-center">BANNER</th>
                  ...
                  <tbody>
                     ...
                     <tr key={index} className="text-center">
                        ...
                        <td className="py-3">{portfolio?.title}</td>
                        <td className="py-3">{portfolio?.description}</td>
                        <td className="py-3">
                           <button
                              onClick={() => toggleOverlay(portfolio.banner)}
                              className="px-4 py-2 font-semibold text-white rounded-md bg-blue-500 hover:bg-blue-600 focus:outline-none"
                           >
                              Show image
                           </button>
                        </td>
                        <td className="py-3">{new Date(portfolio?.created_at).toLocaleString("id-ID")}</td>
                        <td className="py-3 flex justify-center flex-wrap gap-1">
                           <Link to={`edit/${portfolio?.id}`} className="px-4 py-2 font-semibold text-white rounded-md bg-green-500 hover:bg-green-600 focus:outline-none">
                           Edit
                           </Link>
                           <button onClick={() => handleDelete(portfolio?.id)} className="px-4 py-2 ml-2 font-semibold text-white rounded-md bg-red-500 hover:bg-red-600 focus:outline-none">
                           Delete
                           </button>
                        </td>
                     </tr>
