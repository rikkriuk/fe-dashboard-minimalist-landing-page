...
import { fetchTestimonials, deleteTestimonial } from "../redux/slices/testimonialSlice";
import LoadingComponent from "./LoadingComponent";
import { showSuccessAlert } from "../utils/alert";
import OverlayComponent from "./OverlayComponent";

const TestimonialListComponent = ({ toggleOverlay, isOverlayVisible, overlayImageUrl, setIsOverlayVisible }) => {
  const { testimonials, pagination, loading, error } = useSelector((state) => state.testimonial);
  ...
  useEffect(() => {
    dispatch(fetchTestimonials({ search, page, limit: 5 }));
  }, [dispatch, search, page]);

  const handleDelete = (id) => {
    dispatch(deleteTestimonial(id)).then(() => {
      showSuccessAlert();
      dispatch(fetchTestimonials({ search, page, limit: 5 }));
    });
  };
    ...
    <section className="space-y-5 px-20 mt-10">
      <div className="px-6 grid grid-cols-3 gap-4">
        ...
        <div className="flex gap-3 col-span-2 justify-end">
          ...
          />
          <button
            onClick={() => dispatch(fetchTestimonials({ search, page: 1, limit: 5 }))}
                ...
                <table className="w-full text-dark border-neutral-200">
                  <thead className="text-secondary-dark text-[0.95rem] font-semibold">
                    <tr>
                      ...
                      <th className="pb-3 text-gray-600 text-center">FOTO PROFILE</th>
                  ...
                  <tbody>
                    {testimonials?.map((testimonial, index) => (
                      <tr key={testimonial.id} className="text-center">
                        ...
                        <td className="py-3">{testimonial.name}</td>
                        <td className="py-3">{testimonial.title}</td>
                        <td className="py-3 line-clamp-5 w-full">{testimonial.message}</td>
                        <td className="py-3 ">
                            <button
                              onClick={() => toggleOverlay(testimonial.foto_profile)}
                              className="px-4 py-2 font-semibold text-white rounded-md bg-blue-500 hover:bg-blue-600 focus:outline-none"
                           >
                              Show image
                            </button>
                        </td>
                        <td className="py-3">{new Date(testimonial.create_at).toLocaleString("id-ID")}</td>
                        <td className="py-3">{new Date(testimonial.update_at).toLocaleString("id-ID")}</td>
                        <td className="py-3 flex justify-center gap-2">
                          <Link
                            to={`edit/${testimonial.id}`}
                            className="px-4 py-2 font-semibold text-white rounded-md bg-green-500 hover:bg-green-600 focus:outline-none"
                          >
                            Edit
                          </Link>
                          <button
                            onClick={() => handleDelete(testimonial.id)}
                            className="px-4 py-2 font-semibold text-white rounded-md bg-red-500 hover:bg-red-600 focus:outline-none"
                          >
                            Delete
                          </button>
                        </td>
